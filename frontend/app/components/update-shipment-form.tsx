...
export function UpdateShipmentForm({
    ...
    shipmentId,
    ...props
}: { shipment?: ShipmentRead | null, shipmentId?: string | null, onScan: (id: string) => void } & React.ComponentPropsWithoutRef<"div">) {

    // Prefer the id we navigated with (URL param) so the field fills instantly,
    // falling back to the fetched shipment's id.
    const initialId = shipmentId ?? shipment?.id ?? ""
    ...
    const shipments = useMutation({
        ...
        }) => api.shipment.patchShipment({ id }, update),
        onSuccess: () => {
